...
- **Surfaces:** desert ambient (`bg-spheres.jpg`, 0.12 pass, `#050507`) stays visible; one admin window (workspace + rail fused, `--ref-*` frost, 24px, no opaque white left); near-black cards for the Setup board rows. No shadows, no per-cell boxes, no colored badges/status chips. `#adminLayer.open` is transparent (veil retired).
