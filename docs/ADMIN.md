...
- **Classes & Batches (tabbed master-detail, student mirror):** Left bar (compressed 280/240/300px + 24px gutter, ends at the wall) stacks the CLASSES|BATCHES text tabs over one list of 68px frost tiles (copied from `.student-row`, motion stripped) plus the matching input + text Add — one list, one add, one geometry per view, so switching never shifts widths or heights. Class cubes untouched. A 1px wall divides the bar from the compact month view on the right (legend bar with context selector + Prev/Month/Next/Today + 7-column grid, unchanged, auto-compacting). Below, full width, the class/batch solid editor for the selected tile (locked-28px head + hairline + weekday toggles + Present/Late cutoffs + inherit notice + Save, one persist path); first class is default-selected. Tile click swaps the editor only (`renderCubeDetail` into `#scheduleEditor`), same as `selectStudent`. Batch creation lives only in the left bar (BATCHES tab) via the single `submitBatchName` path. New batch names surface in the BATCHES tab until a student carries them into a class.
...
- **Weekly Template, Holidays & Overrides:** Month View (resolved day cells above, editable weekday-header row below) is display-only; weekday toggles and per-context Present/Late timing live in the solid schedule editor. Holiday ranges (`holiday`, `vacation`, `exam`) and single-date overrides are created only from the sidebar (`ADD HOLIDAY`, `ADD OVERRIDE` → same `#holidayModal`/`#overrideModal`), edited/removed through their list tables (table Edit/Remove). The tables live in Holiday / Override popup windows (sidebar eye icons open the same frost modals as creation; `Close` dismisses); Month View is always the workspace. Month cells open the read-only day window; the window offers an `Add override for this date…` shortcut into the override modal (door only — tables stay the single editor).
