...
Unifies school settings, classes, batches, rules, calendar, holidays, and schedule inheritance around two things: the **Month View** workspace (the visual result) and the **Action Wheel** (the command center):
- **Month View (the one workspace):** A viewing surface for the school's schedule across the month — legend bar with context selector + Prev/Month/Next/Today + 7-column grid. The 7-day weekly state row (SUN–SAT, WORKING/NON-WORKING) sits below the date grid as display-only; month cells are inert (no editing entry points). Cells resolve per active context with holiday/override tags plus a `today` ring.
- **Action Wheel (the control center):** Six sectors — Classes, Batches, Schedules, Cutoffs, Exceptions, School Info — each fanning hover actions that land in one workflow each (doors, never duplicate logic).
- **Classes & Batches:** Managed only through the wheel (Manage = open the Schedule window on that tab; + New = open it focused on the add input). The window's registry lists names with student counts, per-row delete, and inline add (single `submitBatchName` / class-POST paths). Selecting a row scopes the editor below it.
- **Schedule workflow (single editor):** The Schedule window (`#schedModal`) holds scope select (Global + classes + batches, mirroring the month context), the registry, 7 staged weekday toggles, per-scope Present/Late cutoffs with inherit notice, and one Save/Cancel. Save persists days + timing through the single `persistCalendar()` path, closes the window, and the Month View re-renders behind it. First class is default-selected.
- **Cutoffs:** Belong to the schedule workflow (per-scope overrides) plus global defaults (wheel Cutoffs → global scope, and School Information). No month strip, no prompt editors, no orphan editors.
- **Exceptions:** Holidays, vacations, and single-date overrides stay separate from the weekly template, managed from the sidebar (ADD HOLIDAY / ADD OVERRIDE + eye record popups) and the wheel Exceptions sector. The Month View reflects them via resolved cell states.
- **School Information:** Basic campus profile and global attendance rules, from the rail (`School Information`) or the wheel (rules / academic-year / PIN entries land in the same window).
- **Precedence (unchanged):** specific-date override → holiday range → weekly template; weekly per-student `Grade|Batch` → batch → class → global; default Sunday off, Mon–Sat on. Formats: `YYYY-MM-DD[..YYYY-MM-DD]:type:name` (`holiday|vacation|exam`, exam = working); overrides `date:1|0:note`.
