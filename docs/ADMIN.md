...
- **Classes & Batches (4-across board row):** Classes · Batches · Holidays · Overrides render as four equal black rectangles under the month (pages flatten so tables grid directly); cards hold a 240px minimum and stretch toward the window bottom, edges flush with the frosted border on the left, right, and bottom (zero frost; corners touch per explicit order). row click retargets the month context (`selKind`/`selName` → month + boards re-render). Creation/editing lives in the per-card EDIT/ADD verbs (`openSchedPopup` + `#holidayModal`/`#overrideModal`, single persist path). Class tiles nest their batches by display regroup only (a batch nests under a class iff ≥1 student carries both). The month view sits above (toolbar with legend + context selector + Prev/Month/Next/Today + 7-column tile grid + working-days readout).
