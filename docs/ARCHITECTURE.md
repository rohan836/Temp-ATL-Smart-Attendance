...
Base palette `bg #FCFBF7 panel #F2F3F6 ink #181A20 ink-2 #6B6B6B ink-3 #A8A5A0 line #E9E6E0 paper #F6F4EF ok #2F5D34 danger #8A3A3A`, Inter/Newsreader/monospace; current paint is the desert-glass reference rollout on top: desert ambient (`bg-spheres.jpg`, `0.12` pass, `#050507` base), warm frost windows (`--ref-frost` / `--ref-frost-blur`, `24px`) for rail/roster/detail/attendance/month/manager/audit, near-black Classes/Batches cards, black-pill primaries + outline pills, flat text-only month cells. The ink toggle flips text tiers only — white cards force `--ref-ink` dark text and black cards keep white text in both modes. Shell layout: top-center search + `248px` right rail (nav + per-tab contexts); `#adminLayer.open` is transparent and kiosk idle chrome hides in Admin. The maintained logic is `ui_app.js`; the HTML stub is replaced at serve time and not edited. Redesign: HTML/CSS in the Production.html, behavior in `ui_app.js`; do not create `css/`/`js/`/`templates/`/components unless proven need. Data load is `cacheLoad() → loadClassesHolidaysSettings() → loadStudents() → loadHistory() → loadTodayAttendance() → cacheSave()` every 15s while visible. LocalStorage omits photos; all writes via API.
