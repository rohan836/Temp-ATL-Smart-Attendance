...
| GET/POST | `/api/settings` | GET open, POST header only | GET omits `sensor/uart/baud/db/host/port/imagesDir/adminPin`. POST whitelist: `schoolName, address, region, academicYear, schoolOpeningDate, attendanceStartDate, presentCutoff, lateCutoff, halfDayCutoff, minPercent, classes, batches, holidays, overrides, workingDays, classSchedules, batchSchedules, schoolLogo, planetImage, heroImage, imageGallery, trajectoryLabels`. Holidays are `YYYY-MM-DD[..YYYY-MM-DD][@HH:MM-HH:MM]:type:name` with `type holiday|vacation|exam` (`exam` is working) as parsed in `app.py`; the optional `@` times are record-keeping (display only — resolution stays day-granular, empty = all-day). At most one `@` segment per record; names/notes containing `@` are rejected (400 / inline error). Overrides use `YYYY-MM-DD[@HH:MM-HH:MM]:0/1:note` or dict form. `workingDays {0..6 bool}` Sun=0. `classSchedules/batchSchedules {key: workingDays}` keys ≤80 (batch keys may be `Class|Batch` composites), ≤50 entries; entries may also carry `presentCutoff`/`lateCutoff` (`HH:MM`, validated, persisted — per-class/per-batch timings). Validates dates/times and `presentCutoff ≤ lateCutoff`. Header-only PIN, no `?pin=` query. |
...
| POST | `/api/backup/gdrive/toggle` | header only | Enable or disable Google Drive backup (`POST {enabled: bool}`). |
