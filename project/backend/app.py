...
def get_db():
    if "db" not in g:
        ...
        if need_init or _schema_missing(g.db):
...
def _schema_missing(db):
    # Heal empty/table-less DB files (created but never seeded, e.g. a
    # connect racing a move/crash): schema is IF NOT EXISTS + OR IGNORE,
    # so re-running on a healthy DB is a safe no-op.
    try:
        row = db.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='settings'").fetchone()
        return row is None
    except Exception:
        return True
...
def settings():
    ...
    if "classSchedules" in j:
        ...
        for k, v in raw.items():
            ...
            if isinstance(v, dict) and "workingDays" in v:
                try:
                    wd = _clean_wd(v["workingDays"])
                except Exception as e:
                    return jsonify({"error":f"bad classSchedules for {key}: {e}"}), 400
                entry = {"workingDays": wd}
                for ck in ("presentCutoff", "lateCutoff"):
                    if v.get(ck):
                        try:
                            datetime.datetime.strptime(str(v[ck]), "%H:%M")
                        except Exception:
                            return jsonify({"error":f"bad classSchedules time for {key}: {ck}"}), 400
                        entry[ck] = str(v[ck])
                cleaned_cs[key] = entry
                ...
                for ck in ("presentCutoff", "lateCutoff"):
                    if v.get(ck):
                        try:
                            ...
                            return jsonify({"error":f"bad batchSchedules time for {key}: {ck}"}), 400
                        entry[ck] = str(v[ck])
                cleaned_bs[key] = entry
            elif isinstance(v, dict):
                try:
