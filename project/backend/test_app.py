...
class ApiTest(unittest.TestCase):
    ...
    def test_production_page_uses_maintained_ui_source(self):
        ...
        self.assertNotIn("bg-spheres.jpg", body)
    ...
    def test_class_batch_schedule_cutoffs_persist(self):
        # Per-class/per-batch cutoffs must survive the settings round-trip
        # (they drive on-time/late per batch); bad times are rejected.
        wd = {str(i): True for i in range(7)}
        r = self.client.post("/api/settings", json={
            "classSchedules": {"Grade 9": {"workingDays": dict(wd), "presentCutoff": "08:00", "lateCutoff": "08:30"}},
            "batchSchedules": {"Grade 9|A": {"workingDays": dict(wd), "presentCutoff": "07:30", "lateCutoff": "08:00"}},
        })
        self.assertEqual(r.status_code, 200)
        s = self.client.get("/api/settings").get_json()
        self.assertEqual(s["classSchedules"]["Grade 9"]["presentCutoff"], "08:00")
        self.assertEqual(s["classSchedules"]["Grade 9"]["lateCutoff"], "08:30")
        self.assertEqual(s["batchSchedules"]["Grade 9|A"]["presentCutoff"], "07:30")
        self.assertEqual(s["batchSchedules"]["Grade 9|A"]["lateCutoff"], "08:00")
        r2 = self.client.post("/api/settings", json={
            "classSchedules": {"Grade 9": {"workingDays": dict(wd), "presentCutoff": "25:00", "lateCutoff": "08:30"}},
        })
