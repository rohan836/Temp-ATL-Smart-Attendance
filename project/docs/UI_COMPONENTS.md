...
108. Unified destinations row + weekday soft-wash (user order): the three
    Backup service rows share one shape — Row-1 checkbox/name/status pill,
    Row-2 optional detail (collapses via `applyDestRowState`), Row-3
    primary/secondary actions. Primary is a solid matte black pill
    (cream text), secondary an outline pill, Clear status red text with
    no pill. A service that cannot act shows its enable/connect primary
    instead of a send action: Drive shows Pair (ready = Send backup
    now / Refresh list / Disconnect), USB shows Check USB (ready =
    Backup to USB / Include), Telegram's primary is always Send.
    Duplicate messaging retired: Drive's static "Authorization required"
    line is gone (status + Pair say it once; the auth card only hosts
    the device-flow code card), and USB's mount line collapses while
    unconnected so the specific error line stands alone. Scheduler
    weekday pills idle as a soft dark wash (`rgba(20,20,20,0.06)` fill /
    `0.18` edge / `0.62` ink, outline pill) so active days read as the
    chosen black-fill pills.
109. Backup footers share one baseline (user order): manager foot and
    audit foot are identical fixed `68px` border-box bars docked to
    the card bottom (`margin: auto -18px -16px`, `padding: 0 18px`),
    so the cream dashed bar and the black divider sit at one vertical
    line across the joint and both 40px button rows center on the same
    baseline at any zoom (U block in Production.html).
110. Audit ink tiers + human details (user order — rows dim, DETAILS
    raw JSON): time/action full `#F4EEE1`, details one step below
    (`rgba(244,238,225,0.8)`), by quiet `0.55`, header quiet-but-legible
    `0.68`, rows hover-off; belt path (`#auditBody td:nth-child`)
    so the older belt-and-braces cream rule cannot outrank it.
    `formatAuditDetails` speaks short words — schedules as
    `Daily · 18:30` / `Mon, Wed · 08:00` / `Every 3 days` / `Off`,
    backups as `Sent <file> · <trigger>`, corrections as
    `sid · date · OLD → NEW · reason`, id-bracket/slot/fid shapes
    likewise, other JSON stripped to `k: v` pairs capped 160 chars.
    Columns, widths, rows, export CSV untouched; red stays Clear-only.
111. Pellet ring era closed + black beds (user order): the Backup ring
    is fully removed (no SVG, WAAPI/SMIL, corner layers, outline, or
    cream — flush black bed, panels edge-to-edge); Setup full-screen
    cream wall; Attendance outlined wall on black bed; Students black
    split gutters + cream panes. Manager scrollbar bar hidden
    (`scrollbar-width:none` + webkit `display:none`), wheel/touch kept;
    audit scroll untouched. Class/batch row tap previews only (pen
    opens the editor — E2E-pinned). All state shifts are layout-free:
    non-working/override/today via inset shadows, OFF header cubes via
    inset 2px on-cube, active cb rows via uniform 400 + inset cream
    bar (weight-flip retired).
112. Boards readability (user order): zero-student rows stay legible —
    dim survives in dot, leader, and count only, never in the name
    (dim names 0.85, dim counts 0.6, empties 0.85, counts 0.7, verbs
    0.8, dim leaders 0.28, dividers cream 0.22, active bar 3px).
    Geometry, markup, and behavior untouched.
113. Botanical month palette (user order): olive `#6B6D43` working cubes
    (cream ink), black non-working (cream ink), terracotta `#CF7D65`
    holidays (cream ink), khaki `#ABA66F` overrides (dark ink); weekday
    header matches (olive cubes, sage `#99B4AA` OFF bars); today ring
    cream except dark on khaki; scope chip olive. Matte red returns to
    destructive-only. Peach reserved unassigned. Paint-only.
114. Palette restricted to three hues (user order, supersedes the
    olive/sage/khaki mapping in 113): pale cream `#F2DEC7` working
    (dark ink), peach `#E1B8A2` OFF bars + overrides (dark ink on
    overrides, cream OFF text), terracotta `#CF7D65` holidays (cream
    ink); non-working stays black. Today ring dark except cream on
    terracotta and black. Scope chip peach with dark text. Paint-only.
115. Setup bed goes warm brown (user order, sampled from reference):
    `#6A482C` bed with `#EAD8BB` sand cubes; terracotta/peach states,
    cream toolbar ink, and black boards unchanged. Paint-only.
116. Classic revert (user order): cream `#F4EEE1` bed, black `#141414`
    working cubes (cream ink), dark-ink OFF framed dark, red-gradient
    holidays + flat `#8A3A3A` overrides (cream ink), dark toolbar ink,
    dark selector divider, frost-pill School button. Boards, rail,
    popups, scope chip, and behavior untouched. Paint-only.
117. Scope chip unfilled (user order): the peach fill read red on
    black — chip wears plain verb paint, scope reads from the
    active-barred class row. Paint-only.
